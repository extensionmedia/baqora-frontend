                ...
                $.get("/search/annonce_category/"+id_annonce_category, function(r){
                    ...
                    $.each(r, function(i, val){
                        $(".args.subCategory").append(`
                            <label for="annonce_category_`+val.id+`" class="inline-block text-sm border bg-red-100 hover:bg-red-300 cursor-pointer rounded-lg px-2 py-1 mb-2"><input type="checkbox" name="subcat" value="`+val.slug+`" id="annonce_category_`+val.id+`" class="mr-1">`+val.annonce_category_name+`</label>
                        `);
                    })
                });
            }
        ...
        $('#search_city').change(function(){
            ...
            if(id_city == '-1'){
                ...
                $.get("/search/city_sector/"+id_city, function(r){
                    ...
                    $.each(r, function(i, val){
                        $(".args.sector").append(`
                            <label for="city_sector_`+val.id+`" class="inline-block text-sm border bg-red-100 hover:bg-red-300 cursor-pointer rounded-lg px-2 py-1 mb-2"><input type="checkbox" name="city_sector" value="`+val.id+`" id="city_sector_`+val.id+`" class="mr-1">`+val.city_sector_name+`</label>
                        `);
                    })
                });
            }
